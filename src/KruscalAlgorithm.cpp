...
void KruscalAlgorithm::insert_node(Node node)
{
    ...
    this->nodes_sets[this->added_nodes].first_insertion(&node);
    ...
    {
        new_edge = this->create_edge(&node, &this->nodes_list[i]);
        this->flattened_adjacency_matrix[matrix_correction + i] = new_edge;
    }
    this->added_nodes += 1;
}

Edge KruscalAlgorithm::create_edge(Node* node0, Node* node1)
{
    Edge new_edge;
    ...
    new_edge.is_drone_covered = false;

    return new_edge;
}

Edge* KruscalAlgorithm::get_mst()
{
    MergeSort::merge_sort(this->flattened_adjacency_matrix, 0, this->added_nodes);
